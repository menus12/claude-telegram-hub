...
| `HUB_OPERATOR_USERNAMES` | csv | — | Operator Telegram usernames (a leading `@` is optional, e.g. `a_gorbachev`) rendered when an agent uses `@operator`. A real `@username` mention trips Telegram's muted-chat "Mentions & Replies" exception, so `@operator` actually notifies the human **in a muted group** — unlike the id-link fallback (a visible badge, no reliable push). Unset = fall back to an id-link mention of `HUB_ADMINS`. Set this per deployment for `@operator` to break mute. |
